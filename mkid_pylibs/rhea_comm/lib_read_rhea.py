...
def insert_data(ret, freq, data, ind=0):
    ...
    #stddata = [np.std(d, ddof = 1)/np.sqrt(10) for d in zip(*data)]
    stddata = [np.std(d, ddof = 1) for d in zip(*data)]
    ret['freq'].append(freq[ind])
    ret['I'   ].append(mdata[ind*2])
    ret['Q'   ].append(mdata[ind*2+1])
    ret['Ierr'].append(stddata[ind*2])
    ret['Qerr'].append(stddata[ind*2+1])
    return ret
...
def read_rhea_swp(fname,ismult=False,miniret=False):
    ...
    for i in range(ch_num):
        ...
        ret[i]['Ierr']    = array(ret[i]['Ierr']).astype(float) / 200000. / (2**28)
        ret[i]['Qerr']    = array(ret[i]['Qerr']).astype(float) / 200000. / (2**28)
        if not miniret:
            ret[i]['IQ']   = ret[i]['I'] + ret[i]['Q'] * 1j
            ret[i]['IQerr']   = ret[i]['Ierr'] + ret[i]['Qerr'] * 1j
