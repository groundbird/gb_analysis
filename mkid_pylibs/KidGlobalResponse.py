...
class _KidGlobalResponse_IQerrData:
    """
    given normal IQ+err data
    :param data: array(0:I, 1:Q, 2:Ierr, 3:Qerr)
    """
    def __init__(self, data):
        self._data = dict()
        self._data['I'] = np.asarray(data[0])
        self._data['Q'] = np.asarray(data[1])
        self._data['Ierr'] = np.asarray(data[2])
        self._data['Qerr'] = np.asarray(data[3])
    def __len__(self):
        return len(self._data['I'])
    def __getitem__(self, key):
        return KidGlobalResponse('I-Q',(np.asarray(self._data['I'][key]),np.asarray(self._data['Q'][key])))
    ...
    def qerr(self):
        return self._data['Qerr']
    def iq(self):
        return self.i() + 1j*self.q()
    def iqerr(self):
        return self.ierr() + 1j*self.qerr()
    def deg(self):
        return np.arctan2(self.q(), self.i()) * 180/np.pi
    def amplitude(self):
        return np.sqrt(self.i()**2 + self.q()**2)
...
_kiddatakinds['I-Qerr']       = _KidGlobalResponse_IQerrData
...
class KidGlobalResponse(object):
    ...
    :param kind: one of ['I-Q', 'I-Qerr', 'dB-DEG', 'I-Q-Gain', 'dB']
