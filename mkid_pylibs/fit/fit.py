...
def _fit(function, xs, ys, err=None, via=None, range=None, silent=True, convert=None, **kws):
    ...
    if err is None:
        ...
        def residue(p, xs):
            v = dict((k, v) for (k, v)   in list(p.valuesdict().items()) if k in list(funcsignature.parameters.keys())[1:])
            return (func(xs,**v) - ys)
            #return np.abs(func(xs,**v) - ys)
    else:
        # w/ err
        def residue(p, xs):
            v = dict((k, v) for (k, v) in list(p.valuesdict().items()) if k in list(funcsignature.parameters.keys())[1:])
            #return np.abs((func(xs,**v) - ys)/err)
            return (func(xs,**v) - ys)/err

    try:
        if err is not None:
            print('--------------scale_covar is False---------------')
            minimized = lmfit.minimize(residue, params, args=(xs,), scale_covar=False, **kws)
        else:
            print('--------------scale_covar is True---------------')
