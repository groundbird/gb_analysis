...
# rhea_comm
RHEA_PATH = Path('/home/gb/controls/rhea_comm')
...
#                   f_centers = klist.tone_freqs,
    ...
    #     else:
    #         swp_data[ki].fitIQ(nfwhm=-1,frqrange = fitconf['frqrange'])
...
# #    is_newblinds = ('blinds_relfreqs' in kldict_new) and (not 'blinds_freqs' in kldict_new)
# #    if is_newblinds:
# #        kldict_new['blinds_freqs'] = [0]*len(klist.blinds_index)
# #        for i, ki in enumerate(klist.blinds_relindex):
# #            kldict_new['blinds_freqs'][i] = kldict_new['kids_freqs'][ki] + kldict_new['blinds_relfreqs'][i] # IF, MHz
# #
    ...
    if not nodb:
        ...
        tod = RheaTod(path=tod_fname, 
                      ...
                      length=length,
                      rate=rate_kHz*1e3,
                      duration=timedelta(seconds=length/(rate_kHz*1000)),
                      sg=klist.sg_freq)
        tod.save()
        meas.tods.add(tod)
        meas.save()
...
if __name__=='__main__':
    ...
    parser.add_argument("length", help="data length [sec]", type=int)    
    ...
    for isect in ini.keys():
        ...
        if confitems['label'] == '': confitems['label'] = confitems['ip_address'].split('.')[-1]

        if "sg_serial" not in confitems: confitems["sg_serial"] = None # backward compatibility
        conf[isect] = confitems

    # run
    for isect,confitems in conf.items():
        sg_config = {}
        ...
        p = Process(target=main, args=(confitems['klist_path'],  confitems['length'],     confitems['fpga'],
