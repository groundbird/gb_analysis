...
class FitResult(object):
    ...
    def report_short_str(self):
        # taken from lmfit's printfuncs.py
        output = ""
        for k,v in self.params.items():
            output += f"{k:_<8} : {v.value: .2e} +/- {v.stderr: .2e} \n"
        return output

    def values(self, with_expr_param=False):
        if with_expr_param:
            return dict([(k, v.value) for (k, v) in list(self.params.items())])
        else:
            return dict([(k, v.value) for (k, v) in list(self.params.items()) if not v.expr])

    def errors(self, with_expr_param=False, prefix='d'):
        if with_expr_param:
            return dict([(prefix + k, v.stderr) for (k, v) in list(self.params.items())])
        else:
            return dict([(prefix + k, v.stderr) for (k, v) in list(self.params.items()) if not v.expr])
    ...
    def dumps(self, **kws):
        ...
        out['function']  = pickle.dumps(self.function,0).decode()
