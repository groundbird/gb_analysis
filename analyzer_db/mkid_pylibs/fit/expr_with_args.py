...
def EWA_to_func(EWA):
    ...
    expr_ = expr.subs(list(zip(arg_symbols, arg_names)))
...
def EWA_to_gradfunc(EWA, useargs=None):
    ...
    gradfuncs = [sympy.lambdify(funcargs,
                                sympy.diff(expr, var).subs(list(zip(arg_symbols, arg_names))),
                                'numpy', dummify=False)
...
def EWA_from_string(string_expr):
    ...
    print(symbols, independent_arg_sym, list(map(type, symbols)))
    ...
    return Expr_with_args(expr, args, [x.name for x in args])
